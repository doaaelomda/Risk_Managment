...
@Component({
  selector: 'lib-shared-dashboard-table',
  imports: [
    CommonModule,
    PrimengModule,
    FormsModule,
    ...
    RadioButtonModule,
    TranslateModule,
    NewTableColumnsComponent,
    NewTableFilterComponent,
  ],
  templateUrl: './shared-dashboard-table.component.html',
  styleUrl: './shared-dashboard-table.component.scss',
})
export class SharedDashboardTableComponent
  implements OnDestroy, OnInit, OnChanges
{
  ...
  private getItemId(item: any): any {
    ...
    try {
      idKey =
        typeof (this as any).entityID_Name === 'function'
          ? (this as any).entityID_Name()
          : (this as any).entityID_Name;
    } catch {
  ...
  onSelectionChange(selection: any) {
    ...
    const prevSelection: any[] =
      Array.isArray(this._prevSelected) && this._prevSelected.length
        ? this._prevSelected
        : Array.isArray(this.selectedItem) && this.selectedItem.length
        ? this.selectedItem
        : [];

    if (!prevSelection.length) {
      this.selectedItem = newSelection;
      this._prevSelected = Array.isArray(this.selectedItem)
        ? [...this.selectedItem]
        : [];
      ...
      if (newSelection.length === 1) {
        this.selected_emiter_checkbox_item.emit({
          action: 'checked',
          item: newSelection[0],
        });
    ...
    const newIds = Array.from(newMap.keys()).filter(
      (id) => id !== null && id !== undefined
    );
    const prevIds = Array.from(prevMap.keys()).filter(
      (id) => id !== null && id !== undefined
    );
    ...
    if (addedIds.length === 1 && removedIds.length === 0) {
      ...
      this.selected_emiter_checkbox_item.emit({
        action: 'checked',
        item: newMap.get(addedIds[0]),
      });
    } else if (removedIds.length > 0 && addedIds.length === 0) {
      // multiple removals
      this.selected_emiter_checkbox_item.emit({
        action: 'unchecked',
        item: prevMap.get(removedIds[0]),
      });
    } else if (addedIds.length > 0 && removedIds.length > 0) {
  ...
  handleSelectedColumnProfile(event: any) {
    ...
    this.profiles[0] = JSON.parse(
      JSON.stringify(this.defultProfileDeep ?? ({} as newProfile))
    );
  }
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (
      changes['isActionShow']?.currentValue !== null &&
      changes['isActionShow']?.currentValue !== undefined
    ) {
    ...
    if (changes['profilesList']?.currentValue) {
      this.profiles = [
        this.defultprofile(),
        ...changes['profilesList'].currentValue,
      ];
      this.defultProfileDeep = JSON.parse(
        JSON.stringify(this.defultprofile() ?? ({} as newProfile))
      );
      const isSelected = this.profiles.find(
        (prof: any) => prof?.profileId == this.selected_profile_column()
      );
      ...
      this.ColumnsFilter = this.selectedProfile?.columns
        ?.filter((col: any) => col?.isSortable)
        .map((col: any) => {
          return { ...col, isChecked: false };
        });
      this.ColumnsFilterTemp = this.selectedProfile?.columns
        ?.filter((col: any) => col?.isSortable)
        .map((col: any) => {
          return { ...col, isChecked: false };
        });
    ...
    if (changes['tableSettings']?.currentValue != null) {
      const tableSettings = changes['tableSettings']?.currentValue;
      this.table_title = tableSettings.title;
      this.showFilters = tableSettings.showFilters;
    }
    ...
    if (changes['dataList']?.currentValue) {
      console.log(changes['dataList'], 'data list changed');
      ...
      if (Array.isArray(this._prevSelected) && this._prevSelected.length) {
        ...
        if (idKey) {
          ...
          this._prevSelected = this._prevSelected.filter((p) =>
            this.data.includes(p)
          );
        }
      }
    }
  ...
  ngOnInit(): void {
    console.log(this.mode(), 'mode here');
  ...
  getCurrentDataEntityFilters(id: number) {
    this._SharedService.getFiltersByDataEntityAndReportID(id , this.reportPartId()).subscribe((res: any) => {
      this.current_data_filter_entity = res?.filters;
      this.filter_profiles = res?.profiles || [];
      this.selected_profile_filter =
        this.filter_profiles.filter(
          (prof: any) => prof?.profileId == res?.defaultProfileID
        )[0] || null;
  ...
  handledselectedFilterProfile() {
    ...
    const form = this.selected_profile_filter?.filters?.map((filter: any) => {
      return {
        filter_type: this.current_data_filter_entity.filter((fil: any) => {
          return fil?.filterId == filter?.filterID;
        })[0],
        firstValue: filter?.firstValue,
        operator: this.all_operators.filter((op: any) => {
          return op?.DataEntityTypeOperatorID == filter?.operatorId;
        })[0],
    ...
    if (this.selected_profile_filter) {
      this._SharedService
        .updateDefultProfile(
          this.filter_id,
          this.selected_profile_filter?.profileId,
          this.reportPartId()
        )
        .subscribe({
          next: (res: any) => {
  ...
  handleSaveProfileFilter() {
    ...
    if (this.update_filter_flag) {
      this._SharedService
        ...
        .pipe(finalize(() => (this.load_save_filter = false)))
        .subscribe({
          next: (res: any) => {
            this.showProfileNameFilter = false;
            this._MessageService.add({
              severity: 'success',
              summary: 'filter profile Updated successfully',
            });
            this.getCurrentDataEntityFilters(this.filter_id);
            this.filter_emitter.emit(this.filterForm.value);
          },
      ...
      this._SharedService
        .saveFilterProfile(req, this.filter_id, this.reportPartId())
        .subscribe({
          next: (res: any) => {
            this._SharedService
              .updateDefultProfile(
              ...
              .subscribe((res) => {
                ...
                this._MessageService.add({
                  severity: 'success',
                  summary: 'filter profile saved successfully',
                });
                this.getCurrentDataEntityFilters(this.filter_id);
                this.filter_emitter.emit(this.filterForm.value);
              });
  ...
  cancelAllFilters(op: any) {
    ...
    this.initFilterForm();
    this._SharedService
      .updateDefultProfile(this.filter_id, null, this.reportPartId())
      .subscribe({
        next: (res: any) => {
          console.log('res update defult', res);
        },
  ...
  handleDeleteColumnProfile() {
    this._SharedService
      .deleteProfileColumns(this.selectedProfile?.profileId)
      .subscribe((res) => {
        const i = this.profiles.findIndex(
          (prof: any) => prof?.profileId == this.selectedProfile?.profileId
        );
  ...
  handleSaveProfile() {
    ...
    const API_CALL$: Observable<any> = this.edit_column_flag
      ? this._SharedService.updateProfileColumns(req)
      : this._SharedService.addNewProfileColumns(req);
    ...
    API_CALL$.subscribe((res) => {
      ...
      this._MessageService.add({
        severity: 'success',
        summary: 'Success',
        detail: this.edit_column_flag
          ? 'Column Profile Updated Successfully'
          : 'Column Profile Added Successfully',
      });
      const new_profile = {
        profileId: this.edit_column_flag
          ? Number(this.selectedProfile.profileId)
          : Number(this.profiles[this.profiles.length - 1].profileId + 14),
        profileName: this.profile_name,
        isDefult: false,
      ...
      if (this.edit_column_flag) {
        const current_index = this.profiles.findIndex(
          (prof: any) => prof.profileId == this.selectedProfile.profileId
        );
  ...
  handleSearchColumnsFilter() {
    this.ColumnsFilter = this.ColumnsFilterTemp?.filter(
      (col: { id: number; label: string }) => {
        return col.label
          .toLocaleLowerCase()
          .includes(this.searchTermSort.toLocaleLowerCase());
      }
    );
  }
