...
@Component({
  selector: 'lib-attachment-control-requirment',
  imports: [
    TranslateModule,
    InputTextModule,
  ...
  templateUrl: './attachment-control-requirment.component.html',
  styleUrl: './attachment-control-requirment.component.scss',
})
export class AttachmentControlRequirmentComponent implements OnInit {
  @Input() id: number = 0;
  ...
  constructor(
    ...
    this._GovernanceService
      .getRequirementControlsById(this.id ?? this.requirementControl)
      .subscribe({
        next: (res: any) => {
          ...
          this._LayoutService.breadCrumbLinks.next([
            ...
            },
            {
              ...
              icon: '',
              routerLink: this.controlId
                ? `/gfw-portal/compliance/overViewDocument/${this.controlId}/content/${this.govControlID}/RequirementControl/${this.requirementControl}/overview`
                : `/gfw-portal/governance/control-management/${this.govControlID}/Requirements/${this.requirementControl}/overview`,
            },
            {
  ...
  getRisksAttachments() {
    ...
    if (this.requirementControl) {
      this._sharedService
        .getNewAttachment(96, this.id ?? this.requirementControl, 95)
        .subscribe({
          next: (res: any) => {
        ...
        .pipe(
          switchMap(() =>
            this._sharedService.getNewAttachment(
              96,
              this.id ?? this.requirementControl ,
              95
            )
          )
        )
