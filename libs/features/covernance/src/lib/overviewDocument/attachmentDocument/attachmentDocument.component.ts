...
@Component({
  selector: 'lib-attachment-document',
  ...
  templateUrl: './attachmentDocument.component.html',
  styleUrl: './attachmentDocument.component.scss',
})
export class AttachmentDocumentComponent implements OnInit {
  ...
  constructor(private _TranslateService :TranslateService,private _LayoutService: LayoutService,
     ...
     private _riskService: RiskService, private _sharedService: SharedService) {
    this.env = enviroment.DOMAIN_URI
    this._ActivatedRoute?.parent?.paramMap.subscribe((res) => {
      this.controlAssessmentID = res.get('Docid');
    })
  }
  ...
  getRisksAttachments() {
    ...
    if (this.controlAssessmentID) {
      this._sharedService
        .getNewAttachment(76, +this.controlAssessmentID,76)
        .subscribe({
          next: (res: any) => {
      ...
      this._ActivatedRoute.parent?.paramMap
        .pipe(
          switchMap(() =>
            this._sharedService.getNewAttachment(76, +this.controlAssessmentID,76)
          )
        )
        .subscribe({
          next: (res: any) => {
  ...
  handleUpdateTitle(newTitle: string) {
    ...
    this._sharedService
      .updateAttachment(this.current_file_update.fileUsageID, newTitle,76)
      .subscribe({
        next: () => {
