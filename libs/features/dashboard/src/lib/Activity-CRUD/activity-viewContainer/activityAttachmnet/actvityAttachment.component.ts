...
@Component({
  selector: 'lib-actvity-attachment',
  imports: [
    TranslateModule,
    InputTextModule,
  ...
  templateUrl: './actvityAttachment.component.html',
  styleUrl: './actvityAttachment.component.scss',
})
export class ActvityAttachmentComponent implements OnInit {
  env: any;
  attachments: any[] = [];
  ...
  dataEntityTypeId: number = 47;
  FEAT_ID!: number;
  fileGroupType: string = '47';
  ...
  constructor(
    ...
    const id = this._activatedRoute.snapshot.parent?.paramMap.get('ActivityId');
    if (!id) return;
    this.FEAT_ID = +id;
  }
