...
@Component({
  selector: 'lib-attachment-incident',
  imports: [
    TranslateModule,
    InputTextModule,
  ...
  templateUrl: './attachmentIncident.component.html',
  styleUrl: './attachmentIncident.component.scss',
})
export class AttachmentIncidentComponent implements OnInit {
    env: any;
  ...
  getRisksAttachments() {
    ...
    if (this.controlAssessmentID) {
      this._sharedService
        .getNewAttachment(38, +this.controlAssessmentID,38)
        .subscribe({
          next: (res: any) => {
      ...
      this._ActivatedRoute.parent?.paramMap
        .pipe(
          switchMap(() =>
            this._sharedService.getNewAttachment(38, +this.controlAssessmentID,38)
          )
        )
        .subscribe({
          next: (res: any) => {
  ...
  handleUpdateTitle(newTitle: string) {
    ...
    this._sharedService
      .updateAttachment(this.current_file_update.fileUsageID, newTitle,38)
      .subscribe({
        next: () => {
