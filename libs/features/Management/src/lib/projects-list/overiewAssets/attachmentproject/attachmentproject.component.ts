...
@Component({
  selector: 'lib-attachment-projects',
  ...
  templateUrl: './attachmentproject.component.html',
  styleUrl: './attachmentproject.component.scss'
})
export class AttachmentprojectsComponent implements OnInit {
  env: any;
  ...
  getRisksAttachments() {
    ...
    if (this.controlAssessmentID) {
      this._sharedService
        .getNewAttachment(29, +this.controlAssessmentID,29)
        .subscribe({
          next: (res: any) => {
      ...
      this._ActivatedRoute.parent?.paramMap
        .pipe(
          switchMap(() =>
            this._sharedService.getNewAttachment(29, +this.controlAssessmentID,29)
          )
        )
        .subscribe({
          next: (res: any) => {
  ...
  handleUpdateTitle(newTitle: string) {
    ...
    this._sharedService
      .updateAttachment(this.current_file_update.fileUsageID, newTitle,29)
      .subscribe({
        next: () => {
