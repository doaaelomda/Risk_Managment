...
@Component({
  selector: 'lib-attachment-initiatives',
  ...
  templateUrl: './attachmentInitiatives.component.html',
  styleUrl: './attachmentInitiatives.component.scss',
})
export class AttachmentInitiativesComponent implements OnInit {
  env: any;
  ...
  getRisksAttachments() {
    ...
    if (this.controlAssessmentID) {
      this._sharedService
        .getNewAttachment(28, +this.controlAssessmentID,28)
        .subscribe({
          next: (res: any) => {
      ...
      this._ActivatedRoute.parent?.paramMap
        .pipe(
          switchMap(() =>
            this._sharedService.getNewAttachment(28, +this.controlAssessmentID,28)
          )
        )
        .subscribe({
          next: (res: any) => {
  ...
  handleUpdateTitle(newTitle: string) {
    ...
    this._sharedService
      .updateAttachment(this.current_file_update.fileUsageID, newTitle,28)
      .subscribe({
        next: () => {
