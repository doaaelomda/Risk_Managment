...
import { NewAttachListComponent, DeleteConfirmPopupComponent } from '@gfw/shared-ui';
...
import { IQuestion } from 'apps/gfw-portal/src/app/core/models/questions.interface';
...
@Component({
  selector: 'lib-questions0control',
  imports: [
    CommonModule,
    RadioButtonModule,
    CheckboxModule,
...
export class Questions0controlComponent {
  constructor(
   ...
   this.questionnareId =  id ? +id : null
    ...
    effect(() => {
      const loading = this._questionnaireS.loadingQuestions();
      this.loading = loading;
    });
  }
    questionnareId!:number|null
  ...
  close(){
    const returnURL = '/gfw-portal/questionnaire/my-questionnaires'
    if(!this.questionnareId)return
    this.closing=true
    this._questionnaireS.closeQuestionnaire(this.questionnareId).pipe(finalize(()=>this.closing =false)).subscribe({
      next:() => {
  ...
  handleSectionChange(section: ISection) {
    console.log(section, 'Section from questions control');
  ...
  getSectionProgress(section: ISection) {
  ...
  current_section_input = input<ISection>();
  current_question_data!: ISection;

  current_selection_question!: IQuestion;
  ...
  changeCurrentQuestion(question: IQuestion) {
    ...
    // dateAnswerTypes
    console.log('current question after setting answer text: ', question);
