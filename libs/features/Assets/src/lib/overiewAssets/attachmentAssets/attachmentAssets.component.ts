...
@Component({
  selector: 'lib-attachment-assets',
  ...
  templateUrl: './attachmentAssets.component.html',
  styleUrl: './attachmentAssets.component.scss',
})
export class AttachmentAssetsComponent implements OnInit {
  breadCrumb:any;
  env: any;
  ...
  constructor(private _TranslateService :TranslateService,private _LayoutService: LayoutService,
     ...
     private _riskService: RiskService, private _sharedService: SharedService,private _AssetsService:AssetsService) {
    ...
    this._ActivatedRoute?.parent?.paramMap.subscribe((res) => {
      ...
      if(this.controlAssessmentID){
        ...
        console.log(this.controlAssessmentID ,'this.controlAssessmentID from service');
          this._AssetsService.getAssetById(this.controlAssessmentID).subscribe({
        next: (res: any) => {
          this.breadCrumb = [
            ...
            },
            {
              ...
              icon: '',
              routerLink: `/gfw-portal/assets/${this.controlAssessmentID}/overview`,
            },
            { name: this._TranslateService.instant('TABS.ATTACHMENTS'), icon: '' },
          ];
  ...
  getRisksAttachments() {
    ...
    if (this.controlAssessmentID) {
      this._sharedService
        .getNewAttachment(16, +this.controlAssessmentID,16)
        .subscribe({
          next: (res: any) => {
      ...
      this._ActivatedRoute.parent?.paramMap
        .pipe(
          switchMap(() =>
            this._sharedService.getNewAttachment(16, +this.controlAssessmentID,16)
          )
        )
        .subscribe({
          next: (res: any) => {
  ...
  handleUpdateTitle(newTitle: string) {
    ...
    this._sharedService
      .updateAttachment(this.current_file_update.fileUsageID, newTitle,16)
      .subscribe({
        next: () => {
