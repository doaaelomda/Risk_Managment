...
@Component({
  selector: 'lib-attachment-threads',
  imports: [
     TranslateModule,
    InputTextModule,
  ...
  templateUrl: './attachmentThreads.component.html',
  styleUrl: './attachmentThreads.component.scss',
})
export class AttachmentThreadsComponent implements OnInit {
  constructor(
    ...
    this._ActivatedRoute?.parent?.paramMap.subscribe((res) => {
      ...
      if (this.controlAssessmentID) {
        ...
        this.threats
          .getThreatById(this.controlAssessmentID)
          .subscribe((res: any) => {
            if (this.controlAssessmentID) {
              this._LayoutService.breadCrumbLinks.next([
...

                {
                  name: res?.data?.name || '-',
                  icon: '',
                  routerLink: `/gfw-portal/Threats-management/${this.controlAssessmentID}/overview`,
                },
                {
        ...
        console.log(
          ...
          'this.controlAssessmentID from service'
        );
      }
    });
  }
  ...
  getRisksAttachments() {
    ...
    if (this.controlAssessmentID) {
      this._sharedService
        .getNewAttachment(49, +this.controlAssessmentID,49)
        .subscribe({
          next: (res: any) => {
      ...
      this._ActivatedRoute.parent?.paramMap
        .pipe(
          switchMap(() =>
            this._sharedService.getNewAttachment(49, +this.controlAssessmentID,49)
          )
        )
        .subscribe({
          next: (res: any) => {
  ...
  handleUpdateTitle(newTitle: string) {
    ...
    this._sharedService
      .updateAttachment(this.current_file_update.fileUsageID, newTitle,49)
      .subscribe({
        next: () => {
