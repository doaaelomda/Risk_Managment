...
@Component({
  selector: 'lib-attachment-process',
  imports: [
    TranslateModule,
    InputTextModule,
  ...
  templateUrl: './attachmentProcess.component.html',
  styleUrl: './attachmentProcess.component.scss',
})
export class AttachmentProcessComponent implements OnInit {
  env: any;
  ...
  constructor(
    ...
    this._ActivatedRoute?.parent?.paramMap.subscribe((res) => {
      this.controlAssessmentID = res.get('processId');
  ...
  getRisksAttachments() {
    ...
    if (this.controlAssessmentID) {
      this._sharedService
        .getNewAttachment(46, +this.controlAssessmentID,46)
        .subscribe({
          next: (res: any) => {
      ...
      this._ActivatedRoute.parent?.paramMap
        .pipe(
          switchMap(() =>
            this._sharedService.getNewAttachment(46, +this.controlAssessmentID,46)
          )
        )
        .subscribe({
          next: (res: any) => {
  ...
  handleUpdateTitle(newTitle: string) {
    ...
    this._sharedService
      .updateAttachment(this.current_file_update.fileUsageID, newTitle,46)
      .subscribe({
        next: () => {
