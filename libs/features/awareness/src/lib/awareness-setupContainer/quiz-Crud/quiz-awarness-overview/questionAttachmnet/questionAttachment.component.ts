...
export class QuestionAttachmentComponent implements OnInit {
  ...
  if (this.audienceId) {
    this.getRisksAttachments();
      ...
      const campaign$ = this.questionId
  ...
  getRisksAttachments() {
    ...
    if (this.questionId) {
      this._sharedService
        .getNewAttachment(59, +this.questionId,59)
        .subscribe({
          next: (res: any) => {
      ...
      this._ActivatedRoute.parent?.paramMap
        .pipe(
          switchMap(() =>
            this._sharedService.getNewAttachment(59, +this.questionId,59)
          )
        )
        .subscribe({
          next: (res: any) => {
  ...
  handleUpdateTitle(newTitle: string) {
    ...
    this._sharedService
      .updateAttachment(this.current_file_update.fileUsageID, newTitle,59)
      .subscribe({
        next: () => {
