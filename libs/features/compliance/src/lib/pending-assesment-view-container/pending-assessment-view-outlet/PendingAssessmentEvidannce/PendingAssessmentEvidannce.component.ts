...
import { AttachUiComponent, InputSearchComponent, NewAttachListComponent, PaginationComponent } from '@gfw/shared-ui';
...
@Component({
  selector: 'lib-pending-assessment-evidannce',
  ...
  templateUrl: './PendingAssessmentEvidannce.component.html',
  styleUrl: './PendingAssessmentEvidannce.component.scss',
})
export class PendingAssessmentEvidannceComponent implements OnInit {
  constructor(private _TranslateService :TranslateService,private _LayoutService: LayoutService,
     ...
     private _riskService: RiskService, private _sharedService: SharedService,private complianceService:ComplianceService, public _PermissionSystemService:PermissionSystemService) {
    this.env = enviroment.DOMAIN_URI
        this.complianceService.controlAssessmentID$.subscribe(id=>{
      this.controlAssessmentID = id;
  ...
  getRisksAttachments() {
    ...
    if (this.controlAssessmentID) {
      this._sharedService
        .getNewAttachment(19, +this.controlAssessmentID,19)
        .subscribe({
      ...
      this._ActivatedRoute.parent?.paramMap
        .pipe(
          switchMap(() =>
            this._sharedService.getNewAttachment(19, +this.controlAssessmentID,19)
          )
        )
        .subscribe({
          next: (res: any) => {
  ...
  handleUpdateTitle(newTitle: string) {
    ...
    this._sharedService
      .updateAttachment(this.current_file_update.fileUsageID, newTitle,19)
      .subscribe({
        next: () => {
